...
class HomeController extends Controller
{
    ...
    {
        // Open source version - set activated status directly without external API
        $response = [
            "active" => "1",
            "username" => trim($request['username'] ?? 'open_source'),
            "purchase_key" => $request['purchase_key'] ?? 'open_source',
            "software_id" => defined('SOFTWARE_ID') ? SOFTWARE_ID : '',
            "domain" => preg_replace("#^[^:/.]*[:/]+#i", "", url('/')),
            "software_type" => $request->get('software_type', 'product'),
        ];
